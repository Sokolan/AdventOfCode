...
void checkIfNeighborAndUpdateDistance(dArr& heightmap, dArr& shortest_distance, point p, point neighbor){
    if((heightmap[p.first][p.second] - heightmap[neighbor.first][neighbor.second]) <= 1 &&
...
int findShortestPath(dArr& heightmap){
    point start(findPoint(heightmap, 'E'));
    if(start == bad_point){
        cout << "NO START POINT" << endl;
        return -1;
    }
    point st(findPoint(heightmap, 'S'));
    heightmap[st.first][st.second] = 'a';

    heightmap[start.first][start.second] = 'z';
    ...
    for(int i = 1 ; i < heightmap.size()*heightmap[0].size() ; ++i){
        ...
        if(heightmap[p.first][p.second] == 'a'){
            return shortest_distance[p.first][p.second];
        }
